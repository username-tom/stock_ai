...
async def _get_today_ib_realized_gain(ib_mode: str) -> float:
    """Return today's realized P&L for IB paper/live mode.

    Computed identically to the ``/api/sandbox/realized-metrics``
    ``daily_realized_pnl`` figure shown in the "Today's Realised P&L" card:
    reconcile IB's per-execution realized PnL into the trade ledger, then sum
    the explicit broker-reported ``Trade.pnl`` for FILLED rows since the start
    of the current UTC day. Crash protection MUST act on the exact same value
    the user sees in the card, so this no longer reads ``ib_service`` executions
    directly (a separate source with a different day boundary that drifts from
    the card) — it mirrors the endpoint's source and boundary precisely.
    """
    ...
    # Match the realized-metrics endpoint exactly: UTC midnight day boundary.
    now_utc = datetime.now(timezone.utc)
    ...
    try:
        async with AsyncSessionLocal() as db:
            # Backfill missing SELL pnl from IB's per-execution realized PnL so
            # the ledger matches TWS — identical to the metrics endpoint.
            try:
                from app.routers.sandbox_router._helpers import reconcile_ib_realized_pnl
                await reconcile_ib_realized_pnl(db, mode)
            except Exception as exc:
                logger.warning("PM crash check: IB realized reconcile skipped: %s", exc)
    ...
    # Sum explicit broker-reported pnl only (no avg-cost reconstruction, which
    # fabricates phantom losses over the incomplete IB fill ledger), for FILLED
    # rows filled/created today (UTC) — the same rows the card totals.
    total = 0.0
    for t in rows:
...
async def _process_ib_engine_signals() -> None:
    ...
    if crash_enabled and crash_value > 0.0 and not crash_triggered_today:
        ...
        # Crash protection acts on today's REALIZED gain (closed-trade P&L) — the
        # same figure shown in the "Today's Realised P&L" card — NOT IB's reqPnL
        # dailyPnL. dailyPnL also folds in unrealized marks, so using it would let
        # real realized losses slip past the kill-switch (and could trip it on
        # transient unrealized swings). This keeps the switch aligned with the
        # documented "daily realized gain" design and with what the user sees.

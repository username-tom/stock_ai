...
export const runAiBotBacktest = (payload) => api.post('/backtest/run-ai-bot', payload).then(r => r.data)
...
export const removeSandboxSymbol = (symbol, profile = undefined) =>
  api.delete(`/sandbox/positions/${symbol}`, { params: profile ? { profile } : {} }).then(r => r.data)
...
export const getAiBotStatus = () => api.get('/sandbox/manager/ai-bot/status').then(r => r.data)
